...
(! ./lost pipeline --generate 1 --pixel-size 5 --focal-length 100 --fov 17 2>/dev/null)
# also throw an error if we get pixel size or focal length without the other:
(! ./lost pipeline --generate 1 --pixel-size 5 2>/dev/null)
(! ./lost pipeline --generate 1 --focal-length 100 2>/dev/null)
