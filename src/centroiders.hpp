...
class CentroidAlgorithm {
 public:
   ...
   * @param image An row-major indexed array of grayscale pixels. 0 is black, 255 is white. The
   * total length is \p imageWidth * \p imageHeight
...
class GreedyCentroidAlgorithm : public CentroidAlgorithm {
 public:
  GreedyCentroidAlgorithm(){};
  Stars Go(unsigned char *image, int imageWidth, int imageHeight) const override;
};

class FloodfillCentroidAlgorithm : public CentroidAlgorithm {
 public:
  FloodfillCentroidAlgorithm(){};
  Stars Go(unsigned char *image, int imageWidth, int imageHeight) const override;
};

/**
 * A simple, fast, and pretty decent centroid algorithm.
 * Simply finds the weighted average of the coordinates of all bright pixels, the weight being
 * proportional to the brightness of the pixel.
 */
...
/**
 * A more complicated centroid algorithm which doesn't perform much better than
 * CenterOfGravityAlgorithm. Iteratively estimates the center of the centroid. Some papers report
 * that it is slightly more precise than CenterOfGravityAlgorithm, but that has not been our
 * experience.
 */
