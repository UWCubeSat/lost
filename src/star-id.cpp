...
StarIdentifiers GeometricVotingStarIdAlgorithm::Go(
    ...
    for (int i = 0; i < (int)stars.size(); i++) {
        ...
        float ra1 = atan(stars[i].x/(camera.xResolution/2.0/tan(camera.xFov/2)));
        float de1 = atan(stars[i].y/(camera.xResolution/2.0/tan(camera.xFov/2)));
        for (int j = 0; j < (int)stars.size(); j++) {
            if (i != j) {
                float ra2 = atan(stars[j].x/(camera.xResolution/2.0/tan(camera.xFov/2)));
                float de2 = atan(stars[j].y/(camera.xResolution/2.0/tan(camera.xFov/2)));
