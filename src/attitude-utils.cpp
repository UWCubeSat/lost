...
float Quaternion::Angle() const {
    // TODO: we shouldn't need this nonsense, right? how come acos sometimes
    // gives nan? (same as in AngleUnit)
    if (real <= -1) {
...
EulerAngles Quaternion::ToSpherical() const {
    // Working out these equations would be a pain in the ass. Thankfully, this
    // wikipedia page:
    // https://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles#Quaternion_to_Euler_angles_conversion
    // uses almost exactly the same euler angle scheme that we do, so we copy
    // their equations almost wholesale. The only differences are that 1, we use
    // de and roll in the opposite directions, and 2, we store the conjugate of
    // the quaternion (double check why?), which means we need to invert the
    // final de and roll terms, as well as negate all the terms involving a mix
    // between the real and imaginary parts.
    float ra = atan2(2 * (-real * k + i * j), 1 - 2 * (j * j + k * k));
    if (ra < 0) ra += 2 * M_PI;
    float de = -asin(
        2 * (-real * j -
             i * k));  // allow de to be positive or negaive, as is convention
...
Quaternion SphericalToQuaternion(float ra, float dec, float roll) {
    ...
    // when we are modifying the coordinate axes, the quaternion multiplication
    // works so that the rotations are applied from left to right. This is the
    // opposite as for modifying vectors.

    // It is indeed correct that a positive rotation in our right-handed
    // coordinate frame is in the clockwise direction when looking down/through
    // the axis of rotation. Just like the right hand rule for magnetic field
...
float DegToRad(float deg) { return deg / 180.0 * M_PI; }

float RadToArcSec(float rad) { return RadToDeg(rad) * 3600.0; }

float ArcSecToRad(float arcSec) { return DegToRad(arcSec / 3600.0); }

float Vec3::MagnitudeSq() const { return x * x + y * y + z * z; }

float Vec2::MagnitudeSq() const { return x * x + y * y; }

float Vec3::Magnitude() const { return sqrt(MagnitudeSq()); }

float Vec2::Magnitude() const { return sqrt(MagnitudeSq()); }
...
float Mat3::At(int i, int j) const { return x[3 * i + j]; }
...
#define _MATMUL_ENTRY(row, col)                                     \
    At(row, 0) * other.At(0, col) + At(row, 1) * other.At(1, col) + \
...
Mat3 Mat3::Transpose() const {
    return {
        At(0, 0), At(1, 0), At(2, 0), At(0, 1), At(1, 1),
        At(2, 1), At(0, 2), At(1, 2), At(2, 2),
    };
}
...
Quaternion DCMToQuaternion(const Mat3 &dcm) {
    // Make a quaternion that rotates the reference frame X-axis into the dcm's
    // X-axis, just like the DCM itself does
    ...
    // we still need to take the cross product, because acos returns a value in
    // [0,pi], and thus we need to know which direction to rotate before we
    // rotate. We do this by checking if the cross product of old and new y axes
    // is in the same direction as the new X axis.
    bool rotateClockwise =
        oldYAxis.crossProduct(newYAxis) * newXAxis > 0;  // * is dot product
    Quaternion yAlign(
        {1, 0, 0}, AngleUnit(oldYAxis, newYAxis) * (rotateClockwise ? 1 : -1));

    // We're done! There's no need to worry about the Z-axis because the
    // handed-ness of the coordinate system is always preserved, which means the
    // Z-axis is uniquely determined as the cross product of the X- and Y-axes.
    // This goes to show that DCMs store redundant information.

    // we want the y alignment to have memory of X, which means we put its
...
long SerializeLengthVec3() { return sizeof(float) * 3; }
