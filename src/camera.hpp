...
class Camera {
   public:
    ...
    Camera(float focalLength, float xCenter, float yCenter, int xResolution,
           int yResolution)
    ...
    Camera(float focalLength, int xResolution, int yResolution)
        : Camera(focalLength, xResolution / (float)2.0,
                 yResolution / (float)2.0, xResolution, yResolution){};

    // Converts from a 3D point in space to a 2D point on the camera sensor.
    // Assumes that X is the depth direction and that it points away from the
    // center of the sensor, i.e., any vector (x, 0, 0) will be at
    // (xResolution/2, yResolution/2) on the sensor.
    Vec2 SpatialToCamera(const Vec3 &) const;
    // Gives /a/ point in 3d space that could correspond to the given vector,
    // using the same coordinate system described for SpatialToCamera. Not all
    // vectors returned by this function will necessarily have the same
    // magnitude.
    ...
    // whether a given pixel is actually in the camera's field of view
